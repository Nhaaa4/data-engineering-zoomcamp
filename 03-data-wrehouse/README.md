...
```sql
SELECT PULocationID 
FROM `bigquery-learning-1770549505.nytaxi.yellow_tripdata`;
...
```
Answer: 0 bytes estimated. Because BigQuery stores table metadata including the total row count. When you execute SELECT COUNT(*), BigQuery doesn't need to scan any actual data.
```